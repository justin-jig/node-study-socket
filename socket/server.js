...
function getUsersInRoom(room) {
    const users = [];
    //찾고자하는 채팅중에 접속한 socket.id값을찾아야함
    ...
    if (clients) {
        clients.forEach((socketId) => {
            ...
            console.log('userSocket.user',userSocket.user);
...
io.on("connection", (socket) => {
    ...
    socket.on("create", (roomName, userName, cb) => {
        ...
        socket.broadcast.emit("newMember", userName);
        cb();
    });
    ...
    socket.on("sendMessage", (message) => {
        ...
        if (message.user === 'all') {
            ...
            if (clients) {
                const userSocket = io.sockets.sockets.get(clients[message.user]);
                userSocket.emit("newSecretMessage", message.message, message.nick, message.nick);
                socket.emit("newSecretMessage", message.message, message.nick, userSocket.user);
            }
        }
    });
